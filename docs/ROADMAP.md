...
| `code` lifecycle skill, generalized (reads repo AGENTS.md for commands/paths) (@main) | In progress | `specs/foundry-core/` | template extraction |
| `spec-reviewer` agent, generalized (entity model read from repo glossary) (@main) | In progress | `specs/foundry-core/` | template extraction |
| `bootstrap` skill: inspect → interview → copy → generate → verify | Planned | `specs/foundry-core/` | template extraction |
...
| COE mechanism: template (landed Wave 2), promote-upstream flow, eval-accretion rule (@main) | In progress | `specs/foundry-core/` | plugin skeleton |
