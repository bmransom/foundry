...
|---|---|---|---|
| foundry-core spec: requirements + design | Validating — awaiting user review of the spec | `specs/foundry-core/` | — |
| Plugin skeleton: marketplace.json, plugin.json, repo self-host scaffold, `check-fast.sh` + self-host byte-identity gate | Ready | `specs/foundry-core/` | spec approval |
| Template extraction from octant: docs.py (parameterized), board.sh, vitepress scaffold, ROADMAP/glossary/spec-conventions/COE templates, githooks, worktree-retire | Backlog | `specs/foundry-core/` | plugin skeleton |
...
| `update` skill: version-marker diff + refresh | Backlog | `specs/foundry-core/` | bootstrap skill |
| Evals L1–L2: fixtures (Rust CLI, Python service, TS monorepo), harness, seeded defects, gate-discrimination checks | Backlog | `specs/foundry-core/` | bootstrap skill |
| Evals L3: spec-reviewer precision/recall suite; lifecycle artifact checks | Backlog | `specs/foundry-core/` | Evals L1–L2 |
| COE mechanism: template, promote-upstream flow, eval-accretion rule | Backlog | `specs/foundry-core/` | plugin skeleton |
