...
| **Manifest** | `.foundry-manifest.json` in a consumer repo: the plugin version, the convention version, plus each installed verbatim file's template name, version, and content hash — how `/foundry:update` tells pristine from customized and whether the repo needs migrating. Package-manager vocabulary (lockfile family). | lockfile (for this file), state file |
