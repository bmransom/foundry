...
| Id | Work | Status | Spec | Depends on |
|---|---|---|---|---|
| debug-skill | debug skill: drive `lldb` to debug native code (C/C++/Rust/Swift) — set/clear breakpoints, step (`step`/`next`/`finish`/`continue`), inspect frames, variables, and backtraces (`frame`, `p`, `bt`), attach to a live process or load a core; harness-agnostic, `gdb` a documented sibling (one skill + an lldb↔gdb map) | Validating — built on `card/debug-skill`, gate green (`check-fast: PASS`); the live `lldb` debugger eval is the pending post-merge check | `roadmap/specs/debug-skill/` | — |
