...
| okf-listing-fidelity | knowledge-listing fidelity (mechanical fixes): lifecycle-aware output — `current` leads, non-current trails with a `(superseded)`/`(historical)` tag across `list` / `index.md` / sidebar; the generated `index.md` declares `okf_version`; the `knowledge/README.md` dead sidebar link is reserved out. Template bumps (`knowledge` v2, `test-knowledge` v2) + eval cases; no convention break | In progress — built on `card/okf-listing-fidelity`, `check-fast: PASS`, `CODE_REVIEW: PASS` (fresh context; fixed non-discriminating tests + seed-index twin + docs-sync) | `roadmap/specs/okf-alignment/` | knowledge-skill |
