...
| lifecycle-autonomy: an autonomy dial for the `code` lifecycle — one level (Supervised/Guided/Autonomous) + a stop-point; set once at Frame, harness-aware (`/loop`, Codex `/goal`) (@claude, branch: feat/lifecycle-autonomy) | In progress — design approved; spec-review CLEAN; implementing | `roadmap/specs/lifecycle-autonomy/` | — |
