...
| self-improving loop S1–S4: signal store → proposer cron → A/B consult UI → deliberate→spec→code pipeline | **In progress** — S1 (loop-signal-store) spec **written + spec-reviewed** (Track B, off `main`); v1 re-scoped across 5 deliberations + a real-data spike; Wave 1 = vertical slice (broker `participant_failed` → S1, surfaces the budget-cap gap); S2–S4 follow (S4 needs code-review) | `roadmap/specs/loop-signal-store/` | spawn-isolation, code-review |
| local multi-repo aggregate: machine-local cross-repo view joining each repo's S1 ledger by `candidate_fingerprint`; `distinct_repo`/`distinct_source_harness` as evidence (never a gate) feeding S2 | **In progress** — spec **written + spec-reviewed** (Track B, off `main`); validated on a real foundry/octant/research-rig dry-run; the S1→S2 bridge | `roadmap/specs/local-multirepo-aggregate/` | self-improving loop S1 |
| repo-local improvement loop: foundry bootstraps a per-repo loop that mines a repo's OWN S1 ledger for repo-SPECIFIC recurring candidates (`distinct_repo=1`, recurring within-repo) and closes them with a repo-local mechanical change — a rule / skill / config / `AGENTS.md` edit IN that repo, never a foundry change. Complements the cross-repo loop: one S1 capture, routed by genericity (cross-repo → promote to foundry; repo-specific → fix in-repo) — the COE-promotion rule's two branches made mechanical. Example: octant's recurring timeout → an octant-local rule, not a foundry edit. | Backlog — **repo-local is the base case** (≈ settled S1 v1 + a repo-local close handler + bounded provenance); shares S1 sensing + the Human-Selection gate + the `candidate_decision` close record with the cross-repo loop, **diverging only at the close action** (deliberated `loop-reuse`). Routing is the Human-Selection judgment, never a mechanical gate. Design after S1 + local-multirepo land; foundry ships the mechanism, each repo runs it on its own ledger (mechanisms-not-content) | spec to write | self-improving loop S1, local multi-repo aggregate |
