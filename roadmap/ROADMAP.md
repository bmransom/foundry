...
| loop-signal-store (S1): two-zone signal store — gitignored raw zone + committed generic candidate/metric ledger; append-only/immutable/rebuildable mechanics; redaction gate with its own discriminating eval; `source_kind` telemetry seam | Ready — spec written 2026-06-20 (deliberated + spec-reviewed); pending design approval, then build | `roadmap/specs/loop-signal-store/` | AppendOnlyStore extraction (separate refactor PR), spawn-isolation |
| self-improving loop S2–S4: proposer cron → A/B consult UI → deliberate→spec→code pipeline | Planned — design/decomposition done; spec to write | spec to write | loop-signal-store (S1), code-review |
