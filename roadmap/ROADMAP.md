...
| okf-listing-fidelity | knowledge-listing fidelity (mechanical fixes): lifecycle-aware output — `current` leads, non-current trails with a `(superseded)`/`(historical)` tag across `list` / `index.md` / sidebar; the generated `index.md` declares `okf_version`; the `knowledge/README.md` dead sidebar link is reserved out. Template bumps (`knowledge` v2, `test-knowledge` v2) + eval cases; no convention break | **Done** — merged to `main` via #61; `check-fast: PASS` (CI), `CODE_REVIEW: PASS` (fresh context; fixed non-discriminating tests + seed-index twin + docs-sync) | `roadmap/specs/okf-alignment/` | knowledge-skill |
| decision-taxonomy | decision-taxonomy: the `decision` type is a misnamed catch-all (four dated genres — COE, review, decision, investigation — and zero ADRs in foundry's own KB), and the taxonomy lacks the evergreen-vs-dated maintenance axis (evergreen concepts edit-to-current; dated records append-and-supersede). Make the axis explicit config, disambiguate the dated genres, document the immutable append/supersede lifecycle. Genre-by-directory (`coes/` / `reviews/` / `decisions/`); `type` set → `reference / architecture / guide / dated` (`decision` retired to the `decisions/` ADR folder, so a COE is `type: dated`, never a "decision"); evergreen-vs-dated via `type` + `dated_types`; append + supersede + extract. Options A (`record` umbrella + field) and B (first-class types) documented as rejected alternatives. Convention break (conv 5) + migration | Planned — design approved (Option C + `type: dated`, 2026-06-29); `SPEC_REVIEW: CLEAN` (fresh context); ready to build (`okf-listing-fidelity` merged) | `roadmap/specs/decision-taxonomy/` | okf-listing-fidelity |
