...
**Status:** Validating (2026-06-10) — tracked on the [board](../../ROADMAP.md).
...
foundry/
  ...
  roadmap/  knowledge/  scripts/  features/  # foundry's own self-hosted setup
```
...
| | `knowledge/glossary.md` stub (debt column; entity-model section empty) | CI workflow running `check-fast.sh` |
...
spec↔board pointers with the board-wins rule; `roadmap/BACKLOG.md` keeps the idea pool off
...
`spec-conventions.md` rule on `roadmap/specs/**` mandating glossary terms and the
