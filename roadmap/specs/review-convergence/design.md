...
- **Objective prose → a linter; subjective and contextual prose → the judge.** A defined
  ...
  (many debt terms have a legitimate non-debt use — "issue" is debt for a board row but fine as
  a GitHub issue), so a deterministic scan cannot tell a violation from legitimate use; it stays
  a `blocking` judge call. Grounded in the documented "a too-subjective rule cannot be enforced" lesson.
...
- **One cross-family pass, two combine-rules — a Strategy, not a shared driver.** The
  ...
  **UNION** (`union`; recall-up, precision-monotone-down — its risk is missing a contract
  violation). For spec-review's `union` use, `footer-algebra.sh` stays **pure set algebra** — the
  spec-review driver derives the `SPEC_REVIEW: CLEAN/FINDINGS` verdict skill-side from the
  surviving set, rather than parameterizing a verdict token into the shared module. (`difference`
  already bundles `CODE_REVIEW: PASS/FAIL` inside the module — a pre-existing exception, left
  as-is; we just don't extend that coupling to spec-review.) The rules are opposite because the failure modes are; each one-directional rule
...
- **No shared review *driver* — compose, don't unify (rejected: Template Method).** A single
  driver over spec-review and code-review was considered and **rejected**: only two uses
  ...
  each skill keeps a **thin driver that composes them**, accepting a little transparent
...
| `evals/fixtures/reviewer/answer-key.json` + `reviewer-eval` | **Changes** — the recall set seeds prose violations (V7 passive/buried-point, V8 needless-qualifier, V9 prose-should-be-table) that the new contract demotes to **advisory**, so they leave the blocking-only footer and `score_review.py` would score them as misses (recall ~0.97 → ~0.7, i.e. 7 of 10 scored). Reassign **V7 and V8** to `prose-lint`'s discrimination set — both seed lintable hedge phrases that the BANNED set catches deterministically (V7's passive hedge and V8's needless qualifier are both in the banned list). **V9** (prose-should-be-table) is the truly *subjective* judge call the linter cannot make (recognizing that an identifier belongs in a table needs judgment) — demote it to **advisory** and drop it from the scored recall set; it has no deterministic home, and that is the point. reviewer-eval recall then measures **blocking contract-violations only**. `score_review.py` itself is unchanged; `spec-convergence-eval.sh` uses an independent grep oracle. |
...
- A full Vale rollout — `prose-lint.py` starts with a small banned-filler-phrase set,
  extensible later.
- Passive-voice detection and deterministic **debt-term** linting — both need context a scan
  lacks (passive is ambiguous; many debt terms have a legitimate non-debt use), so both stay
