...
misses a seeded contradiction the blind pass catches). `prose-lint.py` has its own
discrimination test: a seeded banned phrase exits non-zero, clean prose passes. Runtime: lint
and hook are one-shot parses — perf N/A.
...
- A cross-model refuter for spec-review — `code-review` has one; adding it to spec-review is a
  follow-up, not required to close the COE.
